...
class CasbinRBACEnforcer:
    ...
    Supports dynamic runtime policy mutations, permission changes, and user provisioning.
    """
    def __init__(self, model_path: str = MODEL_PATH, policy_path: str = POLICY_PATH):
        ...
        print("[+] Casbin Zero-Trust RBAC Enforcer initialized with Dynamic Policy Engine.")
    ...
    def add_user_role_mapping(self, username: str, role: str) -> bool:
        """Dynamically assigns a Casbin role to a user."""
        sub = f"user:{username}" if not username.startswith("user:") else username
        r = f"role:{role}" if not role.startswith("role:") else role
        res = self.enforcer.add_role_for_user(sub, r)
        self.clear_cache()
        self.enforcer.save_policy()
        return res

    def add_permission_to_role(self, role: str, resource: str, action: str) -> bool:
        """Dynamically grants an action permission on a resource to a role or user."""
        sub = f"role:{role}" if not role.startswith("role:") and not role.startswith("user:") else role
        res = self.enforcer.add_permission_for_user(sub, resource, action)
        self.clear_cache()
        self.enforcer.save_policy()
        return res

    def remove_permission_from_role(self, role: str, resource: str, action: str) -> bool:
        """Dynamically revokes an action permission on a resource from a role or user."""
        sub = f"role:{role}" if not role.startswith("role:") and not role.startswith("user:") else role
        res = self.enforcer.delete_permission_for_user(sub, resource, action)
        self.clear_cache()
        self.enforcer.save_policy()
        return res
